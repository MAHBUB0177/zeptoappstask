...
<div class="pt-2 px-2">
            ...
            <p class='text-left text-sm'>${book.title}</p>
            <p class='text-left font-semibold text-sm'>${book.authors[0]?.name} (${
    book.authors[0]?.birth_year
  }-${book.authors[0]?.death_year})</p>
            <p class='text-left underline font-semibold '>Genre :</p>
            <ul>
            ${book?.subjects?.map((genre) => `<li class='text-sm'>${genre}</li>`).join("")}
